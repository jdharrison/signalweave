...
fn register_channels(core: &mut SignalweaveCore<DevAuthenticator>) {
    for definition in [
...
fn make_core(config: CoreConfig) -> SignalweaveCore<DevAuthenticator> {
    let mut core = SignalweaveCore::new(authenticator(), config).expect("valid core config");
...
fn connect_authenticated(
    core: &mut SignalweaveCore<DevAuthenticator>,
    token: &str,
) -> signalweave_core::ConnectionId {
...
fn join_and_subscribe(
    core: &mut SignalweaveCore<DevAuthenticator>,
    connection: signalweave_core::ConnectionId,
    space: SpaceId,
) {
...
fn relay_owned_channels_enforce_server_assigned_entity_ownership() {
    ...
    assert!(matches!(
        ...
        Err(CoreError::AuthorityRejected(
            signalweave_core::AuthorityRejection::EntityNotOwned
        ))
    ));
