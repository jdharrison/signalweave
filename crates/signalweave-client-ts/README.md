# Signalweave TypeScript client (WebTransport)

The browser client for Signalweave, generated FlatBuffers bindings plus a real
`WebTransport` transport client that mirrors the reference `signalweave-client-rust`
...
- `src/` — the transport client and framing codec.
  - `src/client.ts` — `SignalweaveClient`: connect, join, subscribe, publish, receive.
...
- `crates/signalweave-protocol/tests/ts_client_wire.rs` decodes frames produced by
  this encoder with the Rust `Codec`, proving the server can consume TS output.

## Using the client (browser)

```ts
import { SignalweaveClient } from "./src/index.js";
...
The standardized Signalweave server URL is `quic://host:port` — a single scheme used by
...
`SignalweaveClient` mirrors `signalweave-client-rust`:
...
```sh
cd crates/signalweave-client-ts
npm ci
...
"$FLATC" --ts -o crates/signalweave-client-ts/generated crates/signalweave-protocol/schemas/signalweave_v1.fbs
```
