...
import { MessageKind, ControlPayload } from "../generated/signalweave/protocol/v1.js";

/** Client connection settings. */
export interface SignalweaveConfig {
  /**
...
/** Errors surfaced by the Signalweave TypeScript WebTransport client. */
export type SignalweaveError =
...
function err(kind: SignalweaveError["kind"], message: string): SignalweaveError {
  return { kind, message };
}
...
/**
 * Signalweave WebTransport client for browsers.
 *
 ...
 */
export class SignalweaveClient {
  ...
  /**
   * Connect to a Signalweave server over WebTransport and complete the protocol
   * handshake.
   */
  static async connect(
    config: SignalweaveConfig,
  ): Promise<SignalweaveClient> {
    ...
    return SignalweaveClient.fromTransport(transport, stream, config);
  }
  ...
  static async fromTransport(
    ...
    const client = new SignalweaveClient(transport, stream);
  ...
  private async handshake(config: SignalweaveConfig): Promise<void> {
    await this.write(
      encodeHello({
        clientName: "signalweave-client-ts",
        clientVersion: "0.1.0",
