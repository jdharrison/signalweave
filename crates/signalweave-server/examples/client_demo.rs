//! High-level end-to-end demo of the Signalweave client story.
//!
...
use signalweave_protocol::{Codec, MessagePayload};
...
/// `signalweave-client-ts` and embedded in `ts_client_wire.rs`; here we run the
...
fn show_ts_frames_decode_in_rust() {
    ...
    // They carry the SWP1 file identifier and the 4-byte size prefix that the
