...
use signalweave_client_rust::{Client, ClientConfig, ClientError};
use signalweave_protocol::{Codec, ControlPayload, MessagePayload, ProtocolErrorCode};
use signalweave_server::development_router;
use tokio::{
...
async fn start_server() -> String {
    ...
    let app = development_router().unwrap();
...
#[tokio::test]
async fn health_and_capabilities_are_public_and_non_stateful() {
    let url = start_server().await;
    let address = url.trim_start_matches("ws://").trim_end_matches("/ws");
    let health = http_get(address, "/healthz").await;
    assert!(health.starts_with("HTTP/1.1 200"));
    let capabilities = http_get(address, "/v1/capabilities").await;
    assert!(capabilities.starts_with("HTTP/1.1 200"));
    assert!(capabilities.contains("\"protocol_version\":1"));
    assert!(capabilities.contains("\"websocket\""));
}
...
#[tokio::test]
async fn entity_owner_is_enforced_over_websocket() {
    let url = start_server().await;
    let config = |url| ClientConfig {
        url,
        token: "dev-token".to_owned(),
...
async fn transition_emits_left_then_entered() {
    ...
    assert!(
        matches!(left.message, MessagePayload::Control(ControlPayload::EntityLeft(ref value)) if left.entity_id == Some(entity) && value.reason == signalweave_protocol::EntityLeaveReason::Transitioned)
    );
    let entered = client.recv().await.unwrap();
    assert!(
...
async fn disconnect_emits_entity_left_to_other_subscribers() {
    ...
    let left = tokio::time::timeout(Duration::from_secs(1), bob.recv())
        ...
        .unwrap();
    assert!(
        matches!(left.message, MessagePayload::Control(ControlPayload::EntityLeft(ref value)) if left.entity_id == Some(alice_entity) && value.reason == signalweave_protocol::EntityLeaveReason::Disconnected)
    );
}

#[tokio::test]
async fn malformed_binary_frame_receives_protocol_error() {
    let url = start_server().await;
    ...
    let Message::Binary(bytes) = frame else {
        panic!("server did not send binary ProtocolError");
    };
    let envelope = Codec::default().decode(bytes.as_ref()).unwrap();
    let MessagePayload::Control(ControlPayload::ProtocolError(error)) = envelope.message else {
