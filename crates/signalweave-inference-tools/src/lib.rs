...
//! calling `signalweave-core` through the same `WorkerHandle` API a transport uses — subject
...
use signalweave_core::{CoalesceKey, Command, ConnectionId, EntityId, SpaceEpoch, SpaceKey};
use signalweave_inference_core::ToolCallProposal;
use signalweave_transport::WorkerHandle;
...
async fn publish_latest_value(
    context: &ToolInvocationContext,
    channel: signalweave_core::ChannelId,
    component: u64,
    sequence: u64,
    payload: Vec<u8>,
) -> Result<(), String> {
    let request = signalweave_core::PublishRequest {
        ...
        channel,
        sequence,
        delivery: signalweave_core::DeliveryClass::LatestValue,
        persistence: signalweave_core::PersistenceClass::Stateful,
...
pub mod demo {
    ...
    use signalweave_core::ChannelId;
    use signalweave_inference_core::ToolCallProposal;
