//! Bounded local load scenarios for Signalweave routing behavior.
...
use signalweave_core::{
    ...
    PublishRateLimit, PublishRequest, RoutingPolicy, SessionId, SessionKey, SignalweaveCore,
    SpaceDescriptor, SpaceEpoch, SpaceId, SpaceKey,
};
...
const PAYLOAD: &[u8] = b"signalweave-load-state";
...
impl From<signalweave_core::CoreError> for LoadError {
...
pub fn run(config: LoadConfig) -> Result<Measurement, LoadError> {
    ...
    let mut core = SignalweaveCore::new(
        authenticator,
        CoreConfig {
